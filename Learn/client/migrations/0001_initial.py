# Generated by Django 3.2.5 on 2022-02-04 13:39

import client.models
from django.db import migrations, models
import django.db.models.deletion
import django_enumfield.db.fields


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='BookApointment',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('user_booking_name', models.CharField(max_length=30)),
                ('user_booking_phone', models.CharField(max_length=30)),
                ('user_booking_datetime', models.DateTimeField()),
                ('user_booking_category', models.CharField(max_length=30)),
                ('user_booking_photographer', models.CharField(max_length=30)),
                ('user_booking_address', models.CharField(max_length=30)),
            ],
            options={
                'verbose_name_plural': 'Book Apointment',
            },
        ),
        migrations.CreateModel(
            name='BookingCategories',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('category_type', django_enumfield.db.fields.EnumField(default=0, enum=client.models.Category_types)),
                ('category_name', models.CharField(max_length=40)),
                ('category_duration', models.DurationField()),
                ('category_price', models.DecimalField(decimal_places=2, max_digits=5)),
            ],
            options={
                'verbose_name_plural': 'Booking Categories',
            },
        ),
        migrations.CreateModel(
            name='Photographers',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('photographer_first_name', models.CharField(max_length=40)),
                ('photographer_last_name', models.CharField(max_length=40)),
                ('photographer_expertise', models.CharField(max_length=40)),
                ('photographer_experience', models.IntegerField()),
                ('photographer_pay', models.IntegerField()),
            ],
            options={
                'verbose_name_plural': 'Photographers',
                'ordering': ['photographer_last_name', 'photographer_first_name'],
            },
        ),
        migrations.CreateModel(
            name='User',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('user_first_name', models.CharField(max_length=50)),
                ('user_last_name', models.CharField(max_length=50)),
                ('user_email', models.EmailField(max_length=30)),
                ('user_password', models.CharField(max_length=100)),
                ('user_account_name', models.CharField(max_length=30)),
            ],
            options={
                'verbose_name_plural': 'User',
                'ordering': ['-user_first_name'],
            },
        ),
        migrations.CreateModel(
            name='WebsiteForm',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('your_name', models.CharField(max_length=50)),
                ('your_email', models.EmailField(max_length=30)),
                ('subject', models.CharField(max_length=200)),
                ('message', models.CharField(max_length=700)),
            ],
            options={
                'verbose_name_plural': 'Website Form',
            },
        ),
        migrations.CreateModel(
            name='UserHistory',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('fulfillment', models.BooleanField()),
                ('order_date', models.DateField()),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='client.user')),
                ('user_service', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='client.bookingcategories')),
            ],
            options={
                'verbose_name_plural': 'User History',
                'db_table': 'UserHistory',
            },
        ),
        migrations.CreateModel(
            name='UserAddress',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('user_city', models.CharField(max_length=30)),
                ('user_district', models.CharField(max_length=30)),
                ('user_country', models.CharField(max_length=50)),
                ('user_zip_code', models.IntegerField()),
                ('user_street', models.CharField(max_length=30)),
                ('user_house_number', models.IntegerField()),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='client.user')),
            ],
            options={
                'verbose_name_plural': 'User Address',
            },
        ),
        migrations.CreateModel(
            name='CurrentBookings',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('fulfillment', models.BooleanField()),
                ('has_paid', models.BooleanField()),
                ('service', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='client.bookingcategories')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='client.user')),
            ],
            options={
                'verbose_name_plural': 'Current Bookings',
            },
        ),
    ]
